...
const createApi = () => {
  ...
  return {
    ...
    getOrders: (id, config) =>
      ...
      getSubscriptionKw: (guid, config) =>
      get(`/api/Subscription/get/kw/${guid}`, config),

    
    getLegalDocument: (orderId, config) =>
      get(`api/LegalDocument/Get/${orderId}`, config),
    getInvoice: (id, year, config) =>
      get(`api/Invoice/GetInvoice/Get/year/${year}/${id}`, config),
    ...
    getSubscription: (id, config) =>
      get(`api/Subscription/${id}`, config),
    getDataCustomer: (dni, config) =>
      get(`api/Contact/GetCustomer/${dni}`, config),
